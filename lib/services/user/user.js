...
const send_verification_code_1 = require("../../hooks/send-verification-code");
...
const user = (app) => {
    ...
    app.service('user').hooks({
        ...
        before: {
            all: [
                schema_1.hooks.validateQuery(user_schema_1.userQueryValidator),
                schema_1.hooks.validateData(user_schema_1.userDataValidator),
                schema_1.hooks.resolveQuery(user_schema_1.userQueryResolver),
                schema_1.hooks.resolveData(user_schema_1.userDataResolver)
            ],
            create: [(0, generate_id_1.generateId)(10)]
        },
        after: {
            all: [schema_1.hooks.resolveResult(user_schema_1.userResolver), schema_1.hooks.resolveExternal(user_schema_1.userExternalResolver)],
            create: [create_wallet_1.createWallet, send_verification_code_1.sendVerificationCode]
        },
        error: {
            all: []
        }
