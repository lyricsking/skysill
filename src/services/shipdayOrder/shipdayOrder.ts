// For more information about this file see https://dove.feathersjs.com/guides/cli/service.html
import { authenticate } from '@feathersjs/authentication'

import { hooks as schemaHooks } from '@feathersjs/schema'

import {
  shipdayOrderDataValidator,
  shipdayOrderPatchValidator,
  shipdayOrderQueryValidator,
  shipdayOrderResolver,
  shipdayOrderExternalResolver,
  shipdayOrderDataResolver,
  shipdayOrderPatchResolver,
  shipdayOrderQueryResolver
} from './shipdayOrder.schema'

import type { Application } from '../../declarations'
import { ShipdayOrderService, getOptions } from './shipdayOrder.class'

export * from './shipdayOrder.class'
export * from './shipdayOrder.schema'

// A configure function that registers the service and its hooks via `app.configure`
export const shipdayOrder = (app: Application) => {
  // Register our service on the Feathers application
  app.use('shipdayOrder', new ShipdayOrderService(getOptions(app)), {
    // A list of all methods this service exposes externally
    methods: ['find', 'get', 'create', 'patch', 'remove'],
    // You can add additional custom events to be sent to clients here
    events: []
  })
  // Initialize hooks
  app.service('shipdayOrder').hooks({
    around: {
      all: [
        authenticate('jwt'),
        schemaHooks.resolveExternal(shipdayOrderExternalResolver),
        schemaHooks.resolveResult(shipdayOrderResolver)
      ]
    },
    before: {
      all: [
        schemaHooks.validateQuery(shipdayOrderQueryValidator),
        schemaHooks.resolveQuery(shipdayOrderQueryResolver)
      ],
      find: [],
      get: [],
      create: [
        schemaHooks.validateData(shipdayOrderDataValidator),
        schemaHooks.resolveData(shipdayOrderDataResolver)
      ],
      patch: [
        schemaHooks.validateData(shipdayOrderPatchValidator),
        schemaHooks.resolveData(shipdayOrderPatchResolver)
      ],
      remove: []
    },
    after: {
      all: []
    },
    error: {
      all: []
    }
  })
}

// Add this service to the service type index
declare module '../../declarations' {
  interface ServiceTypes {
    shipdayOrder: ShipdayOrderService
  }
}
